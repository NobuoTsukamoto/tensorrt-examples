...
def append_fused_argmax(graph):
    ...
    batch, h, w, num_class = graph.outputs[-1].shape
    scale_factor = 2

    # Add resize ope
    conv_outputs = [node for node in graph.nodes if node.op == "Add"][-1].outputs[0]
    ...
    resize_size_input = gs.Constant(
        "resize_size",
        values=np.array([batch, num_class, h // scale_factor, w // scale_factor], dtype=np.int64),
    )
    resize_attrs = {
    ...
    resize_output = gs.Variable(
        name="resize_output",
        dtype=np.float32,
        shape=(batch, num_class, h // scale_factor, w // scale_factor),
    )
    resize_node = gs.Node(
        op="Resize",
        attrs=resize_attrs,
    ...
    argmax_output = gs.Variable(
        name="argmax_output", dtype=np.int64, shape=(batch, 1, h // scale_factor, w // scale_factor)
    )
    argmax_node = gs.Node(
        op="ArgMax",
        attrs=argmax_attrs,
    ...
    cast_output = gs.Variable(
        name="cast_output", dtype=np.float32, shape=(batch, 1, h // scale_factor, w // scale_factor)
    )
    cast_node = gs.Node(
        op="Cast",
        attrs=cast_attrs,
